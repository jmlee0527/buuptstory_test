...
export function ArticleBody({ blocks }: { blocks: ArticleBlock[] }) {
  return <div className="article-prose">{blocks.map((block, index) => {
    ...
    if (block.type === "cta") return <aside key={index} className="paper-card my-9 bg-[#f4f1df] p-6"><h2 className="!mt-0 !text-xl">{block.title}</h2><p>{block.description}</p><Link href={block.href} className="paper-button mt-3 inline-flex min-h-12 items-center bg-primary px-5 text-sm font-black text-white hover:bg-[#36588f]">{block.label}</Link></aside>;
