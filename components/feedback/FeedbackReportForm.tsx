...
export function FeedbackReportForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-5 p-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] sm:p-7">
    <div><label htmlFor="feedback-type" className="text-sm font-black text-ink">{t("feedback.type")} <span className="text-rose-500">*</span></label><select data-feedback-initial id="feedback-type" value={type} onChange={(event) => setType(event.target.value as ReportType)} className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-4 font-semibold text-ink focus:outline-none focus:ring-2 focus:ring-blue-500">{REPORT_TYPES.map((item) => { const typeKeys: Record<ReportType, TranslationKey> = { "개선 의견": "feedback.type.improvement", "문제 및 정답 오류": "feedback.type.answer", "기능 작동 오류": "feedback.type.function", "기타": "feedback.type.other" }; return <option key={item} value={item}>{t(typeKeys[item])}</option>; })}</select></div>
    <div><div className="flex items-center justify-between gap-3"><label htmlFor="feedback-content" className="text-sm font-black text-ink">{t("feedback.content")} <span className="text-rose-500">*</span></label><span className="text-xs font-bold text-slate-400">{content.length}/1,000</span></div><textarea id="feedback-content" aria-required="true" maxLength={FEEDBACK_LIMITS.maximumContent} value={content} onChange={(event) => setContent(event.target.value)} placeholder={t("feedback.contentPlaceholder")} rows={6} className="mt-2 min-h-36 w-full resize-y rounded-xl border border-slate-200 px-4 py-3 leading-6 text-ink focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
    <div><label htmlFor="feedback-contact" className="text-sm font-black text-ink">{t("feedback.contact")} <span className="font-medium text-slate-400">{t("feedback.optional")}</span></label><input id="feedback-contact" type="text" maxLength={FEEDBACK_LIMITS.maximumContact} value={contact} onChange={(event) => setContact(event.target.value)} placeholder="example@email.com" className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 px-4 text-ink focus:outline-none focus:ring-2 focus:ring-blue-500" /><p className="mt-2 text-xs leading-5 text-slate-500">{t("feedback.contactHelp")}</p></div>
    ...
    <p className="rounded-xl bg-amber-50 p-4 text-xs font-semibold leading-5 text-amber-900">{t("feedback.safety")}</p>
    ...
    <button type="submit" disabled={submitting} className="min-h-12 w-full rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-3.5 font-black text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-60">{submitting ? t("feedback.submitting") : t("feedback.submit")}</button>
  </form>;
}
