...
export function FootballTacticsResult({ grade, score, total, wrong, encodedAnswers }: Props) {
  ...
  const scorePercent = Math.round((shareScore / 12) * 100);
    ...
    <div className="min-h-screen pb-24 pt-8 sm:py-14">
      <div className="container-page">
        ...
        <div className="mx-auto max-w-4xl">
            ...
            <div className="bg-[#f4f1df] px-6 pb-7 pt-10 sm:pt-14">
              ...
              <div className="mx-auto mt-7 grid size-36 place-items-center rounded-full p-2 shadow-card" style={{ background: `conic-gradient(#4267A8 ${hasResult ? scorePercent : 100}%, #DDE7F0 0)` }}>
                <div className="grid size-full place-items-center rounded-full bg-[#FFFDF6]">
                  <span><strong className="block text-4xl font-black text-[#4267A8]">{hasResult ? shareScore : `${grade.minScore}~${grade.maxScore}`}</strong><span className="mt-1 block text-[11px] font-bold text-slate-400">{hasResult ? "/ 12점" : "점 구간"}</span></span>
                </div>
              </div>
          ...
          {hasResult && wrong.length === 0 && (
            <section className="paper-card mt-8 bg-[#CDEFE1]/35 p-6 text-center sm:p-8"><h2 className="text-xl font-extrabold text-emerald-900">🎉 전 문제 정답!</h2><p className="mt-2 text-sm leading-6 text-emerald-900">12문제를 모두 맞혔습니다. 선수 움직임과 공간, 압박 구조까지 완벽하게 읽어냈습니다.</p></section>
          )}

          {!hasResult && (
            <section className="paper-card mt-8 p-6 sm:p-8">
              <h2 className="text-xl font-extrabold text-ink">축구 전술 이해도 등급표</h2>
              <p className="mt-2 text-sm text-slate-500">정답 개수에 따라 0점부터 12점까지 계산하고 6개 등급으로 나뉩니다.</p>
          ...
          <section id="share-card" className="mt-10 grid scroll-mt-24 gap-6 rounded-md bg-ink p-6 text-white sm:p-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
            <ShareImageCard emoji={grade.icon} eyebrow="나의 축구 전술 레벨은" title={grade.name} subtitle={grade.summary} badge={`전술 이해도 ${shareScore}/12점`} accent="green" />
            <div><h2 className="text-xl font-extrabold">친구와 진짜 전술 축잘알을 가려보세요</h2><p className="mt-2 text-sm leading-6 text-slate-300">같은 12문제라도 다시 도전하면 새로운 조합이 출제됩니다.</p><div className="mt-5"><ShareButtons title={shareTitle} description={grade.summary} path={sharePath} /></div></div>
          </section>
