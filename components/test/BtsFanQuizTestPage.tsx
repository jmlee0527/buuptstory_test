        ...
        <header className="mb-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-xl font-black sm:text-2xl" style={{ color: theme.text }}>나는 진짜 ARMY일까?</h1>
            </div>
            <strong className="shrink-0 rounded-full bg-white px-3 py-1.5 text-sm shadow-sm" style={{ color: theme.mutedText }}>{Math.min(index + 1, BTS_FAN_QUIZ_SIZE)} / {BTS_FAN_QUIZ_SIZE}</strong>
          </div>
        ...
        {!current ? (
          <section className="grid min-h-72 place-items-center rounded-[2rem] border bg-white/90 p-10 shadow-2xl" style={{ borderColor: theme.border, boxShadow: `0 24px 50px ${theme.shadow}` }} aria-busy="true">
            <div className="text-center">
              <span className="text-5xl" aria-hidden="true">{btsFanTest.icon}</span>
